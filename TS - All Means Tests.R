...
#Examples
scores <- c(20, 50, 80, 15, 40, 85, 30, 45, 70, 60, 90, 25, 40, 70, 65, 70, 98, 40, 65, 
            60, 35, 50, 40, 75, 65, 70, 20, 80, 35, 68, 70, 60, 70, 80, 98, 10, 40, 63, 
            75, 80, 40, 90, 100, 33, 36, 65, 78, 50)

categ <- c("Rotterdam", "Haarlem", "Diemen", "Rotterdam", "Haarlem", "Diemen", 
...
fisher = meansTest(scores, categ, test='fisher')
fisher
cochran = meansTest(scores, categ, test='cochran')
cochran
welch = meansTest(scores, categ, test='welch')
welch
james0 = meansTest(scores, categ, test='james', order=0)
james0
james1 = meansTest(scores, categ, test='james', order=1)
james1
james1Appr = meansTest(scores, categ, test='james', order=1, iters=TRUE)
james1Appr
james2 = meansTest(scores, categ, test='james', order=2)
james2
james2Alt = meansTest(scores, categ, test='james', order=2, alt=TRUE)
james2Alt
james2Appr = meansTest(scores, categ, test='james', order=2, iters=TRUE)
james2Appr
james2ApprAlt = meansTest(scores, categ, test='james', order=2, iters=TRUE, alt=TRUE)
james2ApprAlt
box = meansTest(scores, categ, test='box')
box
scottSmith = meansTest(scores, categ, test='scott-smith')
scottSmith
bf = meansTest(scores, categ, test='brown-forsythe')
bf
ag = meansTest(scores, categ, test='alexander-govern')
ag
mbf = meansTest(scores, categ, test='mehrotra')
mbf
ham = meansTest(scores, categ, test='hartung-agac-makabi')
ham
hamAlt = meansTest(scores, categ, test='hartung-agac-makabi', alt=TRUE)
hamAlt
ok = meansTest(scores, categ, test='ozdemir-kurt')
ok
okAppr = meansTest(scores, categ, test='ozdemir-kurt', iters=TRUE)
okAppr
